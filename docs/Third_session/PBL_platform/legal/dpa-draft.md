...
3. **The Terms of Service, which DO govern us, contain no Art. 28 terms at all.** Verified by full-text search rather than inferred: zero instances of "Data Protection Agreement", "DPA", "processor", "controller", "GDPR" or "data protection". The ToS names other instruments where they displace it — *"If you have signed up for GitHub Enterprise Cloud, the Enterprise Cloud Addendum applies to you"* — and says nothing equivalent for any other plan.
4. **GitHub's own Privacy Statement casts it as CONTROLLER, not processor**, for users accessing the service directly; it becomes a processor only where *"a school or employer supplies your GitHub account"*, governed by a DPA. Our workflows are direct use.

⚠️ **A PAID TEAM PLAN WOULD PROBABLY NOT FIX THIS, and buying one to find out would be a guess.** The Terms of Service govern paid Organizations by their own terms (*"A paid Organization may only provide access to as many Personal Accounts as your subscription allows"*), and the ToS displacement clause names Enterprise Cloud and nothing else. The customer-terms index does say the Customer Agreement covers *"customers who purchase directly from GitHub"*, which pulls the other way — but it is written around corporate purchasing, and the former **"DPA for Non-Enterprise Customers" was RETIRED**, which is hard to reconcile with self-serve tiers being intended to have DPA coverage.

⚠️ **Do not act on the claim that the DPA covers "GitHub Enterprise Cloud, GitHub Enterprise (Unified), GitHub Teams, and GitHub Copilot."** That sentence recurs in web search summaries and was chased to its supposed source, the GitHub Enterprise Cloud Product Specific Terms, which does **not** contain it — that document says only *"The terms of the Data Protection Agreement and the GitHub Privacy Statement apply to the Online Service"*, with "Online Service" defined circularly as what the customer subscribes to under **that** agreement. The product list appears in no GitHub document located. It is unsourced.

**SO THE GAP CANNOT BE CLOSED CONTRACTUALLY ON THIS PLAN.** What remains is architectural or accepted:
- **GitHub Enterprise Cloud** would work — its Product Specific Terms expressly apply the DPA — but it is priced per user per month and is plainly disproportionate for a platform currently holding four sessions.
- **Remove GitHub from the processing**: move the retention jobs to an EEA host (Scaleway is already a papered sub-processor) or a self-hosted runner. Either also removes the US transfer, and a self-hosted runner additionally keeps the admin credential off third-party hardware — which, given the correction above, is the larger exposure.
- **Accept and record**, noting that the TRANSFER is separately lawful under DPF adequacy and that the deficiency is the Art. 28 contract alone.

⚠️ **If accepted, it must be accepted EXPLICITLY and recorded in the CER dossier and the record of processing — not left implicit here.** A known, documented, proportionate gap on a pre-pilot research platform is a defensible position; the same gap undocumented is not. Revisit before any real-participant cohort.
...
- **Why the BACKUP moved to Scaleway too (2026-09-01).** Not a preference: GCS
  became unwritable when the project returned to the Spark plan, and both halves
  were checked rather than assumed — the existing bucket returns *"The billing
  account for the owning project is disabled in state closed"*, and the Firebase
  default bucket returns *"The specified bucket does not exist"* (it was never
  provisioned, and provisioning one also needs billing). Between 2026-08-27 and
  2026-09-01 **the nightly purge deleted sessions with no archive standing behind
  it.** Deletion is the legal duty and kept running, which was the right call, but
  it left no recovery path. The backup-purge interlock stayed DISARMED throughout
  precisely because arming it would have blocked deletion and turned a
  missing-archive problem into a storage-limitation breach.
  ⚠️ **Arm it (`CLEANUP_REQUIRE_BACKUP=1`) once a backup has actually succeeded —
  not before.** Arming it against a destination that has never worked would stop
  the purge on its first run.
