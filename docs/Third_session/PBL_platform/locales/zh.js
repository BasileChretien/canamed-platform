...
(function (root, factory) {
  ...
  "lobby.consent-transcript": "我另外同意，将以视频会议方式举行的课程的书面转录文字以及音频/视频录制内容保存并用于 CaNaMED 研究分析。转录文字由视频会议工具（Microsoft Teams）自动生成，并在我的每一次发言旁标注我的姓名；除转录文字外，课程的录制内容同样会被保存。两者均在成果发表后保存 5 年，访问权限与研究数据集的其余部分同样受限，我可以随时要求删除。不会拍摄任何照片，我也不必打开摄像头。即使不勾选这第三个选项、也不勾选上面的研究选项，我仍然可以参加工作坊——拒绝不会对我的参与、成绩或在校情况产生任何影响。",
  "lobby.consent-version": "告知版本 PIS v11 · 2026-09。",
  "lobby.consent-version-link": "完整隐私政策",
  "lobby.consent-version-suffix": "。",
  "lobby.privacy.summary": "您的数据如何被使用(加入前请阅读)",
  "lobby.privacy.controller-default": "CaNaMED 研究团队(<strong>卡昂诺曼底大学 × 名古屋大学</strong>,GDPR 第 26 条意义上的共同管理者 / APPI 第 27 条第(5)款意义上的共同使用者)",
  "lobby.privacy.controller-named": "<strong>{name}</strong>(本场会话的个人信息管理者)",
  "lobby.privacy.p1": "{controller}会收集您的名字或昵称、大学、年级和自我评估的英语水平。您的姓名对同房间的所有参与者可见,并会显示在您所贡献的分数旁边。",
  ...
  "lobby.privacy.p3": "数据存储于 Google Firebase 实时数据库的 <strong>europe-west1(比利时,欧盟)</strong>。对于日本参与者而言,这是一次跨境传输,受欧盟–日本相互充分性决定(PPC,2019)的保护。实时会话数据将在会话结束后 30 天内清除(若会话始终未结束,则在创建后 90 天内清除);如您勾选了研究同意项,您的贡献将在发表后以与您关联(可识别身份)的形式保留最多 5 年。此外,副本还会在每晚备份中保留至创建后最多 90 天,因此某场会话的最后一份副本约在结束 120 天后才会消失;备份仅用于故障后恢复服务,不用于查询数据。",
  "lobby.privacy.transcript": "课程通过<strong>Microsoft Teams 视频会议</strong>举行，使用您所在大学的订阅——就此而言，<strong>Microsoft</strong> 与 Google 一样是受托处理方。该工具会<strong>自动生成书面转录文字</strong>，并在您的每一次发言旁标注您的姓名；除转录文字外，课程的音频/视频录制内容同样会被保存。转录文字与录制内容<strong>仅在您勾选第三个同意选项时</strong>才会保存——在成果发表后保存 5 年，访问权限与研究数据集的其余部分同样受限，您可以随时要求删除。<strong>不会拍摄任何照片，您也不必打开摄像头。</strong>",
  ...
  "splash.create.sections-help": "今天的各节（按进行顺序）。开场与总结始终包含在内。",
  "splash.create.sections-empty": "No sections yet — add at least one.",
  "splash.create.sections-add-label": "添加单个章节",
  "splash.create.sections-add": "+ 添加一个章节",
  "splash.create.case-label": "病例",
  "splash.create.case-help": "选择一个病例，勾选今天要进行的部分并添加。您仍可在下方调整顺序或删除，也可混入其他病例的部分。",
  "splash.create.case-add": "+ 添加勾选的部分",
  "splash.create.case-full": "已添加 {wanted} 个中的 {added} 个 — 一次课程最多 {max} 节",
  "splash.create.sections-full": "课程已满 — 最多 {max} 节",
  ...
  "stage.label.welcome": "欢迎",
  "stage.label.wrapup": "总结",
  "stage.label.section": "第{n}节 — {title}",
  "stage.label.0": "欢迎",
  "stage.label.4": "总结",
