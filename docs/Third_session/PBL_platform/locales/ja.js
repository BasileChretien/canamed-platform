...
(function (root, factory) {
  ...
  "verify.not-found": "この検証IDは公開レジストリに登録されていません。CaNaMED証明書に記載されたものである場合は、IDと証明書に記載された氏名を添えてCaNaMEDチームへご連絡ください — マスター記録はチームが保有しています。",
  ...
  "lobby.consent-verification": "さらに、私の証明書が独立して検証可能であることに同意します:証明書に印字された検証IDが、氏名とセッションの一方向ハッシュとともに別の登録簿に保存され、第三者が私の検証IDと証明書上の氏名の両方を提示した場合に、CaNaMEDサイト内の公開ページで本物のCaNaMED証明書と一致するかを確認できるようになります。ページは氏名を公開せず、両方の情報が入力された場合にのみ「有効」または「一致しません」とだけ表示します。登録簿は最長10年保管され、いつでも自分のエントリの削除を求めることができます (プライバシーポリシー第18条参照)。この3つ目のチェックを入れなくてもワークショップに参加し、研究に同意することができます。",
