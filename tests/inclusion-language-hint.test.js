 ...
 * shown on the working-hypotheses input (not just the two group-answer cards).
 */
...
test("the language hint is shown on every free-text contribution point", () => {
  ...
  // + the working-hypotheses input = at least 3.
  const n = INDEX.split("answer-input-language-hint").length - 1;
  assert.ok(n >= 3,
    "expected the language hint on >= 3 contribution points (group answers A+B, hypotheses); got " + n);
});

test("the hint sits with the working-hypotheses input", () => {
