...
test("i18n: English is the canonical key set", () => {
  ...
  for (const k of enKeys) {
    assert.match(k, /^(lang|a11y|splash|lobby|waiting|data-rights|stage|room|admin|closed|ended|debrief|impact|tour|test|survey|offline|modal|privacy|verify|rcol|findings|prompts|reset|settings|modA|modB|coach|reference-section|reference|backToTop|answer|endpoll)\./, `key ${k} should be section.prefixed`);
  }
});
...
test("i18n English-only: workshop chrome renders English even under fr/ja", () => {
  ...
  i18n.setLang("en"); // reset shared module state
});
