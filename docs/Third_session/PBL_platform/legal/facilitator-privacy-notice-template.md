...
- **No advertising cookies and no analytics trackers.** But the platform's core
  ...
  **No cookies were observed** — checked on the live site, 22 August 2026: `document.cookie` was empty on the application page. *Read that precisely: it covers script-visible cookies on our own origin during the loads tested. It cannot see `HttpOnly` cookies, nor anything set on `gstatic.com` or `identitytoolkit.googleapis.com`, so those are not evidenced either way. What IS certain about reCAPTCHA is stronger and comes from elsewhere: no request reached any reCAPTCHA endpoint at all, so it cannot have set anything.*
...
| **Google (software libraries and sign-in endpoint)** — the core JavaScript is downloaded from `gstatic.com` on every load of the **application page** and the certificate-verification page (not the privacy or compliance pages), and `identitytoolkit.googleapis.com` is contacted on every visit for the anonymous sign-in | Your IP address and browser details | [TO VERIFY — served from a global network, not region-pinned] | Content delivery |
