...
— you cannot name an importer whose identity varies per request.
Since 21 August 2026 the request is **pinned to OVHcloud, Gravelines, France**,
so the importer is nameable and the disclosure is possible. **That does not
clear the leg**: naming a recipient is not the same as holding a contract with
it, Hugging Face remains a separate recipient in the middle, and the runtime
confirmation is still outstanding. Under APPI the entrustment (委託) carve-out in
