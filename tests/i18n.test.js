...
test("i18n: English is the canonical key set", () => {
  ...
  for (const k of enKeys) {
    assert.match(k, /^(lang|a11y|splash|lobby|waiting|data-rights|stage|room|admin|closed|ended|debrief|tour|test|offline|modal|privacy|rcol|findings|prompts|reset|settings|modA|modB)\./, `key ${k} should be section.prefixed`);
  }
});
