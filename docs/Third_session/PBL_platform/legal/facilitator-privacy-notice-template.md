...
| **What we then store about you from that sign-in** — your email address and the account identifier Google returns | Google (Firebase Authentication), as **our processor** | Firebase Authentication holds the account record so you can sign back in and keep your history | Covered by the **same Google route as every other Google service we use**, and that route is now identified rather than guessed at: the **Firebase Data Processing and Security Terms**, which incorporate the EU **Standard Contractual Clauses, Module 2 (controller-to-processor)** with Google LLC as importer. Google LLC is separately certified under the **EU–US Data Privacy Framework** (effective 23 August 2025). **These are two distinct routes, not one** — the DPF is an adequacy-based solution for transfers to a certified US entity, the SCCs are an Art. 46 safeguard, and Firebase applies the SCCs as the fallback where the DPF does not cover a given transfer. Say which one carries THIS transfer, do not cite both as if they were interchangeable. `[TO VERIFY — for project canamed-69785: acceptance of the Firebase terms, the contracting Google entity, and which of the two instruments is relied on. Same single Google question as Annex IV.]` *(2026-08-20: the mechanism was researched and named, replacing a bare [TO VERIFY]. Naming it is not the same as evidencing acceptance, so a narrower [TO VERIFY] remains — and "Google's own terms" is still not a mechanism.)* |
