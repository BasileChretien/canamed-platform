...
- The tractable fix is to **pin the Hugging Face account to a single named
  ...
  **Pinning is a supported feature, not a question for Hugging Face (researched
  ...
  produces today’s varying recipient. EU-resident options include Scaleway (FR),
  OVHcloud AI Endpoints (FR), Nscale, Nebius (NL) and Public AI (CH), so an EU pin
  removes the onward transfer rather than merely documenting it.
...
| EEA → US (Google: reCAPTCHA, the global Hosting edge, **and Firebase Authentication**). *Firebase Auth holds the signed-in participant's email and account id. Unlike the Realtime Database — pinned to `europe-west1` in `firebase-config.js` — nothing in the configuration pins an Auth location, and Firebase Auth is not regionalised the same way, so this store is presumed outside the EEA. **This was absent from the draft entirely until 2026-08-19**; the pack referenced the auth UID as an identifier five times without ever saying where the account record lives. `[TO VERIFY — the Auth data location for this project.]`* *Cloud Functions and Cloud Storage are **no longer** on this row — the proxy moved to `europe-west1` and the archive bucket is `europe-west1`; corrected 2026-08-19, which NARROWS this transfer rather than removing it* | **Firebase Data Processing and Security Terms**, which incorporate the EU **Standard Contractual Clauses, Module 2 (controller-to-processor)** — you the exporter, **Google LLC** the importer — plus the UK International Data Transfer Addendum for UK-subject transfers. Google LLC is **additionally certified under the EU–US Data Privacy Framework** (effective 23 August 2025, also covering the UK Extension and the Swiss–US DPF), adopted by Google as an Alternative Transfer Solution since September 2023. Sources: firebase.google.com/terms/firebase-sccs-eu-c2p, firebase.google.com/terms/firebase-mcc, policies.google.com/privacy/frameworks | **INSTRUMENT IDENTIFIED — acceptance for THIS project still owed (2026-08-20).** What research settled is that the mechanism **exists and is published**, not that it was accepted here: clause 12.4 still carries `[TO VERIFY]` for acceptance by project `canamed-69785`, the contracting Google entity, and the SCC module actually in force. Record those three, with dates, before this row may read as executed. *(This cell previously read "[TO VERIFY] … not executed as far as this draft can evidence", which conflated "we have not looked it up" with "it does not exist"; it was then briefly marked ESTABLISHED, which conflated "the instrument exists" with "we accepted it". Both are overstatements in opposite directions.)* |
| EEA → Hugging Face (the router) | **Hugging Face DPA**, publicly offered (cdn-media.huggingface.co/landing/assets/Data+Processing+Agreement.pdf). Note also that **Hugging Face SAS has its main establishment in Paris, France** — an EU establishment with CNIL as lead supervisory authority (huggingface.co/privacy) | **STILL BLOCKING, but no longer UNKNOWN — researched 2026-08-20.** This row read "None identified", which was a statement about the draft's research, not about the world: a DPA exists. That changes the task from "discover whether anything is possible" to "execute a published instrument" — but an instrument that exists and an instrument that binds us are different things, so the row stays blocking until acceptance is evidenced. Record the executed version and its effective date here |
| EEA → onward inference provider via Hugging Face | **Solvable by configuration — the recipient CAN be pinned.** Hugging Face Inference Providers accepts an explicit `provider` (or a `:provider` suffix on the model id); the default is `auto` / `:fastest`, which is what makes today's recipient vary per request. **EU-resident providers are available** — Scaleway (FR), OVHcloud AI Endpoints (FR), Nscale, Nebius (NL), Public AI (CH). Pinning an EU provider removes the onward transfer from the EEA altogether; pinning a non-EU one at least makes the importer nameable and assessable | **BLOCKING UNTIL PINNED — reframed 2026-08-20.** Previously "impossible until the account can be pinned"; pinning is a supported feature, not a request to Hugging Face. `functions/index.js` currently calls `router.huggingface.co` unpinned and reads the provider *after the fact* from the `x-inference-provider` response header, which is precisely the condition described. Source: huggingface.co/docs/inference-providers |
