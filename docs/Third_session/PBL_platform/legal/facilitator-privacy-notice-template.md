...
| **What we then store about you from that sign-in** — your email address and the account identifier Google returns | Google (Firebase Authentication), as **our processor** | Firebase Authentication holds the account record so you can sign back in and keep your history | Covered by the **same Google route as every other Google service we use**: the **Firebase Data Processing and Security Terms**, which incorporate the EU **Standard Contractual Clauses, Module 2 (controller-to-processor)** with Google LLC as importer. Google LLC is also certified under the **EU–US Data Privacy Framework** (effective 23 August 2025). It is *not* a separate mechanism, and "Google's own terms" would not be one. *(Researched and cited 2026-08-20; this cell previously carried a [TO VERIFY].)* |
