...
| 4 | **The inference provider behind Hugging Face** — **OVHcloud AI Endpoints** (OVH SAS) once the pending deploy lands; **varies per request** in the deployed version | Actual model execution | Same as #3 | **Pinned in code to `ovhcloud`, served from Gravelines, FRANCE — but NOT YET DEPLOYED.** The running function is still unpinned, so the CURRENT recipient varies per request and is presumed US. `functions/index.js` sets `HF_PROVIDER=ovhcloud`; it takes effect only on `firebase deploy --only functions` | **Art. 28 foreign provision.** The recipient becomes nameable when the pin is deployed, which is what Art. 28(2) requires and what this row previously could not supply. **Still not authorised under clause 6.1**: pinning names the recipient, it does not by itself produce the contract or the transfer record, and the deploy has not happened. Confirm post-deploy from `metrics/hfPatient/events` (`provider` field, taken from the `x-inference-provider` response header) |
