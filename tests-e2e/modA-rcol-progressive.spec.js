 ...
 *
 * UX de-clutter (2026-06-01) + Debate/answers MERGE (2026-06-25): Module A's
 * right column now has TWO tabs — "Decide together" and "Debate & answers" (the
 * old standalone "Debate" discussion-prompt tab was merged into the answers
 * panel, which now holds the two questions). They reveal one per phase:
 *
 *   - at entry (nothing gathered)        → the whole right column is collapsed;
 *   - history ≥1 AND exam ≥1             → "Decide together" appears;
 *   - ≥1 working hypothesis (phaseGateOpen) → "Debate & answers" appears.
 *
...
test.describe("Module A — right-column tabs reveal one per phase", () => {
  test("at entry the whole right column is collapsed (no tabs shown)", async ({ page }) => {
    ...
    // The standalone Debate tab was merged away.
    await expect(page.locator("#rcol-tab-discussion")).toHaveCount(0);
  });
  ...
  test("'Decide together' appears once the patient is interviewed AND examined", async ({ page }) => {
    ...
    // Debate & answers tab stays hidden.
  ...
  test("the merged 'Debate & answers' tab appears once the first hypothesis is written", async ({ page }) => {
    // Gate threshold is ≥1 hypothesis. The Debate + answers are now ONE tab
    // (#rcol-tab-answers) that reveals as soon as the gate opens.
    await surfaceModA(page);
    const allIds = await page.evaluate(() =>
      window._test_getItemIds ? window._test_getItemIds() : []);
    await reveal(page, allIds);
    // Zero hypotheses → still hidden (revealing the chart must NOT open it).
    await page.evaluate(() => {
      window._test_setHypotheses({});
      if (window.revealModARightCol) window.revealModARightCol();
    });
    await expect(page.locator("#rcol-tab-answers")).toBeHidden();
    // One hypothesis crosses the gate → the merged tab reveals (no lock teaser).
    await page.evaluate(() => {
    ...
    // The merged panel carries the two questions (diagnosis + culture inputs).
    await expect(page.locator("#answer-input-moduleA-diagnosis")).toHaveCount(1);
    await expect(page.locator("#answer-input-moduleA-culture")).toHaveCount(1);
  });
  ...
  test("the mobile thumb-reach bar mirrors the reveal (no premature Debate & answers)", async ({ page }) => {
    ...
    // No hypotheses yet → the phase gate is closed → the merged tab mirror stays hidden.
    expect(s.answers, "Debate & answers mirror stays hidden before any hypothesis").toBe(true);
  });
});
